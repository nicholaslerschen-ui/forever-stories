  ...
  try {
    ...
    const existingResponse = await pool.query(
      `SELECT pr.*, p.question, p.category 
       ...
       AND (pr.response_type = 'daily' OR pr.response_type = 'text')
       LIMIT 1`,
      [userId, today]
    );
...
app.post('/api/prompts/respond', authenticateToken, async (req, res) => {
  try {
    const { promptId, response, isFollowUp, parentResponseId, isBonus } = req.body;
    const userId = req.user.userId;
    ...
    const responseType = isBonus ? 'bonus' : 'daily';
    ...
    const result = await pool.query(
      `INSERT INTO prompt_responses (user_id, prompt_id, prompt_text, response_text, response_type, created_at)
       ...
       RETURNING *`,
      [userId, promptId, '', response, responseType]
    );
