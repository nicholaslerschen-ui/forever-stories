...
app.get('/api/prompts/today', authenticateToken, async (req, res) => {
  try {
    ...
    // Use PostgreSQL's CURRENT_DATE in user's timezone to get today's date
    const existingResponse = await pool.query(
      `SELECT pr.*, p.question, p.category 
       FROM prompt_responses pr
       LEFT JOIN prompts p ON pr.prompt_id = p.id
       WHERE pr.user_id = $1 
       AND (pr.created_at AT TIME ZONE 'UTC' AT TIME ZONE $2)::date = (CURRENT_TIMESTAMP AT TIME ZONE $2)::date
  ...
  try {
    ...
    const streakResult = await pool.query(
      `WITH RECURSIVE date_series AS (
        ...
        SELECT DISTINCT (created_at AT TIME ZONE 'UTC' AT TIME ZONE $2)::date as response_date
        FROM prompt_responses
        WHERE user_id = $1
