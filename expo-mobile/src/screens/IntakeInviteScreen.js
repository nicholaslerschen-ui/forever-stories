...
export default function IntakeInviteScreen({ navigation }) {
  ...
  const handleContactSelect = (contact) => {
    ...
    // Find first empty slot or add new field
    const firstEmptyIndex = contacts.findIndex(c => !c.trim());

    if (inviteMethod === 'email' && email) {
      if (firstEmptyIndex !== -1) {
        updateContact(firstEmptyIndex, email);
      } else if (contacts.length < 10) {
        setContacts([...contacts, email]);
      }
    } else if (inviteMethod === 'sms' && phone) {
      if (firstEmptyIndex !== -1) {
        updateContact(firstEmptyIndex, phone);
      } else if (contacts.length < 10) {
        setContacts([...contacts, phone]);
      }
    } else {
      // Switch method if needed
      if (email && inviteMethod === 'sms') {
      ...
      } else if (phone && inviteMethod === 'email') {
        setInviteMethod('sms');
        if (firstEmptyIndex !== -1) {
          updateContact(firstEmptyIndex, phone);
        } else if (contacts.length < 10) {
