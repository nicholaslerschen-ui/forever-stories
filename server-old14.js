...
app.get('/api/prompts/today', authenticateToken, async (req, res) => {
  try {
    ...
    const today = userTime.toISOString().split('T')[0]; // YYYY-MM-DD

    // Check if user already answered TODAY'S DAILY prompt (not bonus prompts)
    // Convert database timestamps to user's timezone before comparing dates
    const existingResponse = await pool.query(
      `SELECT pr.*, p.question, p.category 
       FROM prompt_responses pr
       LEFT JOIN prompts p ON pr.prompt_id = p.id
       WHERE pr.user_id = $1 
       AND DATE(pr.created_at AT TIME ZONE 'UTC' AT TIME ZONE $3) = $2
       AND pr.response_type = 'daily'
       LIMIT 1`,
      [userId, today, userTimezone]
    );
  ...
  try {
    ...
    const streakResult = await pool.query(
      `WITH RECURSIVE date_series AS (
        ...
        SELECT DISTINCT DATE(created_at AT TIME ZONE 'UTC' AT TIME ZONE $2) as response_date
        FROM prompt_responses
        WHERE user_id = $1
      ...
      streak_counter AS (
        ...
        SELECT 
          ...
          1 as streak_day
        FROM date_series
        WHERE response_date = (CURRENT_TIMESTAMP AT TIME ZONE $2)::date
